...
class InventreeApp(ttk.Window):
    ...
    def _create_top_frame(self, parent):
        ...
        details_frame = ttk.Labelframe(
            top_frame, text="Item Details / Add New Stock", padding=10
        )
        details_frame.pack(side='left', fill='x', expand=True)

        labels = [
            "Item Name:", "Supplier:", "Location:", "Stock to Add/Set:",
            "Low Stock Level:", "Purchase Price (₹):", "Sale Price (₹):"
        ]
        self.entries = {}
        entry_keys = [
            'item', 'supplier', 'location', 'stock',
            'low_stock', 'purchase_price', 'sale_price'
        ]
    ...
    def _create_tree_frame(self, parent):
        ...
        columns = (
            'name', 'stock', 'low_stock', 'purchase_price',
            'sale_price', 'supplier', 'location'
        )
    ...
    def _create_dashboard_frame(self, parent):
        ...
        self.total_items_var = tk.StringVar(value="Total Unique Items: 0")
        self.total_value_var = tk.StringVar(value="Total Stock Value: ₹0.00")
        self.low_stock_var = tk.StringVar(value="Items with Low Stock: 0")
        ttk.Label(
            dashboard_frame, textvariable=self.total_items_var, font=("-weight bold")
        ).pack(side='left', padx=10)
        ttk.Label(
            dashboard_frame, textvariable=self.total_value_var, font=("-weight bold")
        ).pack(side='left', padx=10)
        ttk.Label(
            dashboard_frame, textvariable=self.low_stock_var, font=("-weight bold")
        ).pack(side='left', padx=10)
    ...
    def _create_bottom_frame(self, parent):
        ...
        ttk.Button(
            bottom_frame, text="Settings", command=self.open_settings_window,
            style='secondary.TButton'
        ).pack(side='left')
        ttk.Button(
            bottom_frame, text="View History Log", command=self.open_history_window,
            style='secondary.TButton'
        ...
        ttk.Button(
            right_bottom_frame, text="Download Template", command=self.download_template,
            style='secondary.TButton'
        ).pack(side='left', padx=5)
        ttk.Button(
            right_bottom_frame, text="Import from CSV", command=self.import_from_csv,
            style='primary.TButton'
        ).pack(side='left', padx=5)
        ttk.Button(
            right_bottom_frame, text="Export to CSV", command=self.export_to_csv,
            style='secondary.TButton'
        ).pack(side='left')
    ...
    def open_sale_dialog(self):
        ...
        def process_sale():
            ...
            database.log_change(
                item_name, 'SOLD',
                f"{qty_to_sell} units sold. Stock: {current_stock} -> {new_stock}."
            )
        ...
        ttk.Button(
            button_frame, text="Confirm Sale", command=process_sale, style="success.TButton"
        ).pack(side='left', padx=10)
    ...
    def refresh_data(self):
        rows = database.fetch_inventory(
            self.sort_column, self.sort_direction, self.search_entry.get()
        )
    ...
    def add_item(self):
        ...
        try:
            ...
            Messagebox.show_error(
                "Stock and Price must be valid numbers.", title="Input Error"
            )
            return
        ...
        if existing_item:
            ...
            new_avg_price = (
                (current_stock * current_avg_price) +
                (stock_to_add * purchase_price_new)
            ) / new_stock
            ...
            Messagebox.show_info(
                f"Updated stock for '{vals['item']}'.", title="Stock Updated"
            )
            self.check_and_notify(
                current_stock, new_stock, int(vals['low_stock'] or current_stock)
            )
        else:
            ...
            if database.insert_new_item(new_item_vals):
                database.log_change(
                    vals['item'], 'CREATED',
                    f"Item created with stock {stock_to_add}."
                )
                Messagebox.show_info(
                    f"New item '{vals['item']}' added.", title="Item Added"
                )
                self.check_and_notify(float('inf'), stock_to_add, low_stock)
            else:
                Messagebox.show_error(
                    f"An item with the name '{vals['item']}' already exists.", title="Error"
                )
        self.clear_fields()
        self.refresh_data()
    ...
    def update_item(self):
        ...
        if not selected_item:
            Messagebox.show_warning(
                "Please select an item to update.", title="Selection Error"
            )
            return
        ...
        if not vals['purchase_price'] or not vals['location']:
            Messagebox.show_error(
                "Purchase Price and Location cannot be empty.", title="Input Error"
            )
            return
        try:
            ...
            Messagebox.show_error(
                "Stock and Price fields must be valid numbers.", title="Input Error"
            )
            return
        ...
        database.update_item_details(
            original_name, new_stock, new_low_stock, new_purchase_price,
            new_sale_price, vals['supplier'], vals['location']
        )
        self.clear_fields()
        self.refresh_data()
    ...
    def delete_item(self):
        ...
        if not selected_item:
            Messagebox.show_warning(
                "Please select an item to delete.", title="Selection Error"
            )
            return
        item_name = self.tree.item(selected_item, 'values')[0]
        if Messagebox.yesno(
            f"Are you sure you want to delete '{item_name}'?", title="Confirm Delete"
        ) != 'Yes':
            return
    ...
    def populate_fields_on_select(self, event):
        ...
        entry_map = [
            'item', 'stock', 'low_stock', 'purchase_price',
            'sale_price', 'supplier', 'location'
        ]
    ...
    def download_template(self):
        filepath = filedialog.asksaveasfilename(
            ...
            initialfile="inventree_template.csv",
        ...
        try:
            ...
            Messagebox.show_info(f"Template saved to:\n{filepath}", title="Template Saved")
        except Exception as e:
            Messagebox.show_error(f"An error occurred: {e}", title="Error")
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if not name or not row.get('Stock') or not row.get('Purchase Price') or not row.get('Location'):
                        error_count += 1
                        continue
                    if database.fetch_item_by_name(name):
                        skipped_count += 1
                        continue
                    try:
                        stock = int(row['Stock'])
                        ...
                        if database.insert_new_item(item_values):
                            database.log_change(
                                name, 'CREATED',
                                f"Item imported from CSV with stock {stock}."
                            )
                            success_count += 1
                        else:
                            error_count += 1
    ...
    def open_settings_window(self):
        ...
        ttk.Button(
            frame, text="Save", command=save, style='success.TButton'
