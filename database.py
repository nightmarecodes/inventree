...
def log_change(item_name, action, details=""):
    """Log a change to the history_log table."""
...
def fetch_inventory(sort_column, sort_direction, search_query=""):
    """Fetch inventory items, optionally filtered and sorted."""
    query = (
        "SELECT name, stock, low_stock, purchase_price, sale_price, supplier, location "
        "FROM inventory"
    )
    params = ()
    if search_query:
...
def insert_new_item(values):
    ...
    try:
        execute_query(
            "INSERT INTO inventory (name, stock, low_stock, purchase_price, sale_price, supplier, location) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            values
        )
        return True
...
def add_stock_to_item(name, new_total_stock, new_average_price):
    """Update stock and purchase price for an item."""
    execute_query(
...
def update_item_details(name, stock, low_stock, purchase_price, sale_price, supplier, location):
    """Update all details for an inventory item."""
    execute_query(
...
def fetch_history_log():
    """Fetch the history log, most recent first."""
    return execute_query(
...
def fetch_dashboard_stats():
    """Fetch dashboard statistics: total items, total value, low stock count."""
...
def fetch_low_stock_for_email():
    """Fetch items with critical and warning stock levels."""
    critical = execute_query(
        "SELECT name, stock, low_stock FROM inventory WHERE stock <= low_stock",
        fetch='all'
    )
    warning = execute_query(
        "SELECT name, stock, low_stock FROM inventory "
        "WHERE stock > low_stock AND stock <= low_stock * 1.1 AND low_stock > 0",
        fetch='all'
    )
...
def send_low_stock_email():
    ...
    if not sender_email or not password or not recipient_email:
        return (False, "Email not configured in Settings or .env file.")
    ...
    html_body = (
        "<html><body><h2>Inventree Stock Alert</h2>"
...
def update_stock_level(name, new_stock):
    """Directly update the stock level for a given item."""
    execute_query(
